...
class IHyperVAgentManagement(AppAssureAPI):
    ...
    http://docs.appassure.com/pages/viewpage.action?pageId=3965709
    """
    ...
    def getSnapshotsCount(self, virtualMachineId):
        """Get the count of snapshots for virtual machine."""
        return self.session.request('hypervagent/%s/getSnapshotsCount'
                % (virtualMachineId))
    ...
    def pingSession(self, virtualMachineId):
        """Indicates that session is still being used."""
        return self.session.request('hypervagent/%s/pingSession'
                % (virtualMachineId), 'PUT')
